...
export default function InputStop({stopConfg = {}, onChangeStop, verifications = {}}){
    return(
        <div className={styles.input_stop_box}>
            ...
            <div className={styles.confg_stop_box}>
                <div className={styles.stop_box}>
                    ...
                    <InputSimple min={1} style={{gridArea: 'input1'}} label={`Inferior`} onChange={(e) => onChangeStop('genValue', e.target.value)} type={'number'} value={stopConfg?.genValue} stop={true} disabled={!stopConfg?.genActive} verification={verifications.gen}/>
                </div>
                <div className={styles.stop_box}>
                    ...
                    <InputSimple style={{gridArea: 'input1'}} label={`Inferior`} onChange={(e) => onChangeStop('diffValue', e.target.value)} type={'number'} stop={true} value={stopConfg?.diffValue} disabled={!stopConfg?.diffActive} verification={verifications.diff}/>
                </div>
            </div>
        </div>
    )
